...
#=
As `p` is an element of the vector space generated by `basis`, there exist unique coefficients `coeffs` such that `p = sum(coeffs[a]*a for a in basis)`. Now, when I take `b` in basis of highest degree, `c, rem = divrem(p, a)` should return `rem = sum(coeffs[a]*a for a in basis if !(a==b))` and `c = p[b]`, because `b` does not divide any of the other `a`. To see the last assertion, assume `b` divides `a`. Since `b` has maximal degree, `a` has maximal degree, too, and `a = bc` for some constant `c`. This however, contradicts that `p` has a unique representation `p = sum(coeffs[a]*a for a in basis)`, because
```
